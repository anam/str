...
public partial class Default2 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            DataTable dt = HelperClass.getCompanyDetails(id);
            DataTable dt2 = HelperClass.getCompanyNames(id);
            foreach (DataRow row in dt.Rows)
            {
                txtEmail.Text = row["email"].ToString();
                txtManager.Text = row["name"].ToString();
                foreach (DataRow row2 in dt.Rows)
                {
                    txtName.Text = row2["name"].ToString();
                }
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        if (isPass)
        {
            HelperClass.EditCompany(id, name, manager, email, password);
            lblSuccess.Text = name + " company saved.";
        }
    }
}